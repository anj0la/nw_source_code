 ...
 * Information:
 * 
 * This function implements the Needleman_Wunsch algorithm serially. It uses a Cell matrix, holding the score and the direction of the sequence to make the traceback portion of the 
 ...
 * Say we have our executable file compiled called nw_serial, with two text files seq1.txt and seq2.txt and their respective lengths 4 characters, and a match, mismatch and gap penalty score of 1, -1 and -2 respectively.
...
void init_matrix(Cell** matrix, int m, int n, int gap);
void fill_matrix(char* s1, char* s2, Cell** matrix, int match, int mis_match, int gap);
void traceback_matrix(char* s1, char* s2, Cell** matrix);
void nw(char* s1, char* s2, int match, int mis_match, int gap);
...
/**
 * Implements the matrix filling step of the algorithm, by filling in all of the cells in the matric.
 ...
 *       init_matrix(matrix, m, n, gap);
 *       fill_matrix(s1, s2, matrix, match, mis_match, gap);
...
void nw(char* s1, char* s2, int match, int mis_match, int gap) {
    ...
    Cell** matrix = allocate_matrix(strlen(s1), strlen(s2));
